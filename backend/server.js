...
import { findById, findOne, find } from "./models/User";
import { DeliveryCharge, DefaultDeliveryCharge } from "./models/DeliveryCharge";
...
app.use(urlencoded({ limit: "50mb", extended: true })); // For URL-encoded payloads
...
app.get("/api/user", authenticateUser, async (req, res) => {
  try {
    const user = await findById(req.userId).select("email address");
    if (!user) return res.status(404).send("User not found");
    res.json(user);
  } catch (error) {
    res.status(500).send("Server error");
...
app.get("/api/admin/orders", async (req, res) => {
  try {
    const users = await find({ "orders.payment.screenshot": { $exists: true, $ne: "" } })
      .select("name email address orders") // Include the orders array in the response
      .populate({
        path: "orders.cart.itemId", // Correct path for population
        model: "General", // The model to populate
    ...
    switch (status) {
      ...
      case "Dispatched":
        ...
        if (invoiceFile) {
          attachments.push({
            filename: invoiceFile.originalname, // Use the original filename
            path: join(__dirname, 'uploads', invoiceFile.filename), // Full path to the file
          });
        }
        break;
      case "Delivered":
