...
const JournalPage = () => {
  ...
  useEffect(() => {
    axios.get("http://localhost:5001/api/home/journal")
        ...
        <main className="max-w-4xl mx-auto bg-white p-6 shadow-lg rounded-lg my-8">
          ...
          <section className="mt-6">
            {journal.description.map((para, index) => (
              <p key={index} className="text-lg text-gray-700 mb-4">{para}</p>
            ))}
          </section>

          {/* Services */}
          <section className="mt-6">
            <h3 className="text-2xl font-semibold text-green-700 border-b pb-2 mb-3">Our Services</h3>
            <ul className="list-disc ml-6 text-gray-700 space-y-2">
              {journal.services.map((service, index) => <li key={index}>{service}</li>)}
            </ul>
          </section>

          {/* Special Services */}
          <section className="mt-6">
            <h3 className="text-2xl font-semibold text-green-700 border-b pb-2 mb-3">Our Special Services</h3>
            <ul className="list-disc ml-6 text-gray-700 space-y-2">
              {journal.specialServices.map((service, index) => <li key={index}>{service}</li>)}
            </ul>
          </section>

          {/* Journals & Magazines */}
          <section className="mt-6">
            <h3 className="text-2xl font-semibold text-green-700 border-b pb-2 mb-3">Journals & Magazines</h3>
            <p className="text-gray-700">{journal.journalsAndMagazines.overview}</p>
            <h4 className="text-lg font-semibold mt-4">Strategies:</h4>
          ...
          {/* Branches */}
          <section className="mt-6">
            <h3 className="text-2xl font-semibold text-green-700 border-b pb-2 mb-3">Branches</h3>
            {journal.branches.map((branch, index) => (
              <div key={index} className="bg-gray-50 p-4 rounded-md mb-4">
                <h4 className="text-lg font-semibold">{branch.city}</h4>
                <p>📍 {branch.address}</p>
