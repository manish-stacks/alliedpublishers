...
const AdminConferenceBooks = () => {
  ...
  const fetchBooks = async () => {
    setIsLoading(true);
    try {
      const response = await api.get(`/admin/conference/books`);
  ...
  const handleDelete = async (id) => {
    ...
    setIsLoading(true);
    try {
      await api.delete(`/admin/conference/books/${id}`);
  ...
  const handleUpdate = async () => {
    ...
    try {
      await api.put(
        `/admin/conference/books/${editingBook._id}`,
        editingBook
      );
  ...
  const handleInputChange = (e) => {
    ...
    setEditingBook({
      ...editingBook,
      [name]: ["price", "stock", "discount", "pages"].includes(name) ? Number(value) : value,
    });
  };
        ...
        <div className="w-full max-w-6xl mx-auto space-y-8">
          ...
          <div className="bg-white/40 backdrop-blur-md border border-white/30 rounded-lg p-8 shadow-lg">
            <h3 className="text-3xl font-bold text-black uppercase mb-6 text-center">
              Upload Conference Proceedings Books
            </h3>
            ...
            <div className="flex flex-col items-center space-y-6">
              ...
              <button
                ...
                className={`px-6 py-3 text-white font-bold uppercase rounded-lg transition hover:shadow-md hover:scale-105 active:scale-95 ${
                  isLoading || !file ? "bg-gray-400 cursor-not-allowed" : "bg-[#10263e] hover:bg-[#357ABD]"
                }`}
              >
              ...
              <div className="mt-4 text-sm text-gray-600 text-center">
                ...
                <p>
                  Required fields: <span className="font-bold">title</span> and <span className="font-bold">type</span>
                </p>
              </div>
            </div>
          </div>
                    ...
                    (page) => (
                      <button
                        ...
                        className={`px-3 py-1 rounded ${
                          currentPage === page ? "bg-[#10263e] text-white" : "border"
                        }`}
                      >
                        {page}
                      </button>
...
export default AdminConferenceBooks;
