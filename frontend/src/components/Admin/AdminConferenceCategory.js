...
const AdminCategory = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await axios.put(`http://localhost:5001/api/admin/conference-categories/${editingId}`, {
          name: formData.name,
          subcategories: subcategoriesArray
        });
        toast.success('Category updated successfully!');
      } else {
        await axios.post('http://localhost:5001/api/admin/conference-categories', {
          name: formData.name,
          subcategories: subcategoriesArray
        });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:5001/api/admin/conference-categories/${id}`);
