...
const ConferenceCatalogueUpload = () => {
  const [fileUrl, setFileUrl] = useState("");

  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await api.post(`/api/home/admin/catalogue/conf/upload`, { fileUrl });
      alert("Conference Catalogue link saved successfully!");
      setFileUrl("");
    } catch (error) {
      console.error("Error saving Conference Catalogue:", error);
    }
  };
      ...
      <div className="flex flex-1 justify-center items-center px-12 ml-64">
        <div className="w-full max-w-2xl bg-white/40 backdrop-blur-md border border-white/30 rounded-lg p-8 shadow-lg transition-all duration-500 hover:scale-[1.03] hover:shadow-xl">
          <h3 className="text-3xl font-bold text-black uppercase mb-6 text-center">
            Admin Panel - Conference Catalogue Upload
          </h3>
...
export default ConferenceCatalogueUpload;
