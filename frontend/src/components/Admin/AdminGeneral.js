...
const AdminGeneral = () => {
  ...
  const fetchBooks = async () => {
    setIsLoading(true);
    try {
      const response = await api.get(`/admin/general/books`);
  ...
  const handleDelete = async (id) => {
    ...
    setIsLoading(true);
    try {
      await api.delete(`/admin/general/books/${id}`);
  ...
  const handleUpdate = async () => {
    ...
    try {
      await api.put(
        `/admin/general/books/${editingBook._id}`,
        editingBook
      );
  ...
  const handleInputChange = (e) => {
    ...
    setEditingBook({
      ...editingBook,
      [name]: name === "price" || name === "stock" || name === "discount" || name === "pages"
        ? Number(value)
        : value,
    });
  };
